...
* ​**模板标题**​：`通知📢（可随意）`
* ​**模板内容**​：
  标题：<span v-pre>{{title.DATA}}</span>
  内容：<span v-pre>{{content.DATA}}</span>
...
| `url`          | 点击消息后跳转的链接（可选）                                |
| `data.title`   | 对应模板中的<code v-pre>{{title.DATA}}</code>，设置了深蓝色`#173177` |
| `data.content` | 对应模板中的<code v-pre>{{content.DATA}}</code>                          |
